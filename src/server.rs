...
impl Iterator for Server {
	...
	fn next(&mut self) -> Option<Self::Item> {
		match self.try_accept() {
			Ok(req) => Some(req),
			// Invalid input is an error generated by the request parser
			// when the request isn't valid HTTP.
			// This is not a fatal error, so we just skip the request.
			Err(e) if e.kind() == io::ErrorKind::InvalidInput => self.next(),
			Err(e) => {
